...
void Model::loadModel(std::string const &path)
{
    ...
    //aiProcess_CalcTangentSpace应用到Assimp的ReadFile函数时, Assimp会为每个加载的顶点计算出柔和的切线和副切线向量
    const aiScene* scene = importer.ReadFile(path, aiProcess_Triangulate | aiProcess_GenSmoothNormals | aiProcess_FlipUVs | aiProcess_CalcTangentSpace);
    // check for errors
    if(!scene || scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE || !scene->mRootNode) // if is Not Zero
    {
        qDebug() << "ERROR::ASSIMP:: " << importer.GetErrorString() << Qt::endl;
        return;
    }
...
Mesh Model::processMesh(aiMesh *mesh, const aiScene *scene)
{
    ...
    mat.ambient = QVector3D(color.r, color.g, color.b);
    material->Get(AI_MATKEY_COLOR_DIFFUSE, color);
    mat.diffuse = QVector3D(color.r, color.g, color.b);
    material->Get(AI_MATKEY_COLOR_SPECULAR, color);
    mat.specular = QVector3D(color.r, color.g, color.b);
    mat.shininess = 32.0f;
...
unsigned int Model::TextureFromFile(const char *path, const string &directory)
{
    ...
    {
        qDebug() << "Texture failed to load at path: " << path << Qt::endl;
        stbi_image_free(data);
    }
